...
#center the temp:
II_Data$Temperature_centered <- scale(II_Data$Temperature, center=TRUE,scale=FALSE)
II_Data$Age_centered <- scale(II_Data$Age, center=TRUE,scale=FALSE)
...
II_Data %>%
  count(Temperature_centered,CFUs,Age)%>%
  ggplot(aes(x=Temperature_centered,y=CFUs,color=Age))+ 
  scale_shape_identity(guide="legend")+
  geom_point()+
  facet_grid(~Age, labeller = labeller(Age=agelabs, Temperature_centered=templabs))+
...
II_Summary <- II_Data %>%
  group_by(Temperature_centered,Age) %>%
...
II_Summary %>%
  count(Temperature_centered,mean_CFUs,Age,SE_CFUs)%>%
  ggplot(aes(x=Temperature_centered,y=mean_CFUs,color=Age,
             ymin=mean_CFUs-SE_CFUs, ymax=mean_CFUs+SE_CFUs))+
  geom_point()+
  facet_grid(Temperature_centered~Age, labeller = labeller(Age=agelabs, Temperature_centered=templabs))+
...
z3 <- zeroinfl(CFUs ~ Temperature_centered*Age_centered | ## Predictor for the Poisson process
                 1, ## Predictor for the Bernoulli process;
               dist = 'negbin',
               data = II_Data)
summary(z3)

z4 <- zeroinfl(CFUs ~ Temperature_centered*Age_centered | Temperature_centered*Age_centered ## Predictor for the Poisson process
...
z5 <- zeroinfl(CFUs ~ Temperature_centered*Age_centered | Temperature_centered+Age_centered ## Predictor for the Poisson process
               , ## Predictor for the Bernoulli process;
               dist = 'negbin',
               data = II_Data)
summary(z5)
...
z7 <- zeroinfl(CFUs ~ Temperature_centered*Age_centered | Age_centered ## Predictor for the Poisson process
               , ## Predictor for the Bernoulli process;
               dist = 'negbin',
               data = II_Data)
summary(z7)
...
f <- function(data, i) {
  require(pscl)
  z3 <- zeroinfl(CFUs ~ Temperature_centered*Age_centered | 1,
                 dist = 'negbin',
                 data = data[i,],
                 start = list(count = c(12.6751, 0.7089, 0.1585,0.024), 
                              zero = c(-3.4222)))
