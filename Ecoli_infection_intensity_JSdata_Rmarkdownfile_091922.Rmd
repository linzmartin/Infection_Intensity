...
The estimate for temperature is 0.533625. This means that for each one unit increase in temperature, the expected change in log(CFUs) is 0.533625, when age=0 (and there is no interaction). The estimate for age is -0.556265. For each one unit increase in age, the expected change in log(CFUs) is -0.556265, when temperature=0 (and there is no interaction).

The coefficient for a one unit change in temperature only indicates an increase in log(CFUs) of 0.533625 when the value of age is zero, not when all variables are held constant - this is because you cannot hold both the value of main effects and the interaction term constant without changing one or the other (the interaction includes the main effect).

That means that the main effect coefficients are much less informative than they would be if no interaction were present, because we don't see values of age=0 or temperature=0 in our dataset (see Brambor et al 2006).

Temperature and age interact. The reductive effect of age when temperature=0 substantially decreases when temperature increases (there is a positive coefficient on the interaction term).
...
The significant p-value for the intercept for the zero-inflation model indicates that the intercept is non-zero. Therefore, the baseline odds of an excessive zero (zero inflation model) is 1-exp(3.4222)= -0.408. This is an odds ratio because it is a binomial model with logit link.
...
The percent change in the incident rate of CFUs (occurrence of CFUs at any time) changes by 1.705% for every unit increase in temperature when age = 0 and by 0.573% for every unit increase in age when temperature = 0 (when the opposite variable is zero, due to a significant interaction, as in the explanation above).
