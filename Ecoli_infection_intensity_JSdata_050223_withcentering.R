...
##
#center the temp and age variables:
#centering helps when doing regression so that the intercepts make sense in interpreting the data

#save original data first as original
II_Data$Temperature_uncentered <- II_Data$Temperature
II_Data$Age_uncentered <- II_Data$Age

#now center (take average and then subtract individ value):
II_Data$Temperature <- scale(II_Data$Temperature, center=TRUE,scale=FALSE)
II_Data$Age <- scale(II_Data$Age, center=TRUE,scale=FALSE)

str(II_Data)

#change character type to just numeric instead of scaled variable
II_Data$Temperature <- as.numeric(II_Data$Temperature)
II_Data$Age <- as.numeric(II_Data$Age)

str(II_Data)

#round to 2 digits each:
II_Data$Temperature <- round(II_Data$Temperature, 2)
str(II_Data$Temperature)

II_Data$Age <-round(II_Data$Age, 1)
str(II_Data$Age)
###
#how centering works
unique(II_Data$Temperature)
mean(II_Data$Temperature_uncentered) #29.76293 is mean temp (29.7C)
mean(II_Data$Temperature) #mean now approximates zero

unique(II_Data$Age)
mean(II_Data$Age_uncentered) #7.298851 is mean age (7 days)
mean(II_Data$Age) #approximates zero
...
names(templabs)<- c("-2.76","0.24","2.24") #assign centered values

agelabs <- c("1 day","5 days", "10 days", "15 days")
names(agelabs)<-c("-6.3","-2.3","2.7","7.7") #assign centered values
...
z7 <- zeroinfl(CFUs ~ Temperature*Age | Age ## Predictor for the Poisson process
               ...
               data = II_Data)
summary(z7) 
#standard errors of zero model for z7 are super large and unlikely
...
AIC(z3,z4) #z3 has fewer df, relatively similar AIC
...
#be.zeroinfl()
fitbe_z4 <- be.zeroinfl(z4, data=II_Data, 
                        dist="negbin", alpha=0.01, trace=FALSE) 
summary(fitbe_z4)

summaryfitbe_z4 <- summary(fitbe_z4)
#summaryfitbe_z4



capture.output(summaryfitbe_z4, file = "stepwiseselectionmodel.txt")
...
newdata1 <- expand.grid(c(-2.76,0.24,2.24),rep(c(-6.3,-2.3,2.7,7.7)))
...
#newdata1$zinbprobatzero <-predict(z3,newdata1,type="prob",at=c(0,24434,10000,15000,100000,400000))
#str(newdata1)
...
#####
#compare total counts of zero observations for real data vs. diff models:
...
f <- function(data, i) {
  ...
  z3 <- zeroinfl(CFUs ~ Temperature*Age | 1,
                 dist = 'negbin',
                 data = data[i,],
                 start = list(count = c(12.6732, 0.7089, 0.1585,0.024), 
                              zero = c(-3.4222)))
...
#save
library("writexl")
write_xlsx(parms,"zinb_model_estimates_and_CIs.xlsx")
...
## add row names
expparms <- as.data.frame(expparms)
expparms
names(coef(z3))
...
## print results
expparms
write_xlsx(expparms,"zinb_model_estimates_and_CIs_responsescale.xlsx")

#baseline odds ratio for zero-intercept model
#exp(-3.4222)
#########################
newdata1
...
ggplot(newdata1, aes(x = Temperature, y = response, colour = factor(Age))) +
  ...
  #geom_line(aes(x=Temperature,y=count),color="black",data=newdata1)
...
mylist <- list(Temperature=-2.76,Age=2.7)
...
mylist <- list(Age=seq(-6.30,7.70,by=0.5),Temperature=c(templow,tempmid,temphigh))
...
mylist <- list(Temperature=seq(-2.76,2.24,by=0.5),Age = c(ageyoung,agelowmid,agehighmid,ageold))
